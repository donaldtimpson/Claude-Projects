...
function buildComponents(variant: Variant): Components {
  ...
  const section = dark ? "text-gold-300" : "text-zinc-500";
  ...
  return {
    h1: ({ children }) => <h3 className={`text-lg font-bold mt-5 first:mt-0 ${heading}`}>{children}</h3>,
    h2: ({ children }) => (
      <h3 className={`font-display text-sm tracking-[0.15em] uppercase mt-6 first:mt-0 ${section}`}>{children}</h3>
    ),
    h3: ({ children }) => <h4 className={`font-semibold mt-4 ${heading}`}>{children}</h4>,
    p: ({ children }) => <p className={`text-sm leading-relaxed my-3 ${body}`}>{children}</p>,
    ul: ({ children }) => <ul className={`list-disc pl-5 space-y-1.5 my-3 text-sm ${body}`}>{children}</ul>,
    ol: ({ children }) => <ol className={`list-decimal pl-5 space-y-1.5 my-3 text-sm ${body}`}>{children}</ol>,
