...
/// Shuffles inside each group but never across them.
///
/// A fixed order gets memorised: a child starts reciting from position rather than
/// reading the card, so decks reshuffle each time they are opened. But the phonics
