...
This is the **web app**, at `Claude-Projects/edu-web/` (renamed from `edu-platform`; the Vercel
project's Root Directory must be `edu-web`). A native **iOS app** lives in the sibling
`Claude-Projects/edu-ios/` (SwiftUI). Both are in the one git repo (root = `Claude-Projects`).

The iOS app consumes a **token-based mobile API** exposed here under `app/api/mobile/v1/*`
(Bearer JWT via `lib/mobile-auth.ts`, reusing the same `User` store as web NextAuth through
`lib/auth-core.ts`; shared engagement logic in `lib/services/activity.ts`; offline-write dedup via
`lib/mobile/idempotency.ts` + the `IdempotencyKey` table). This is separate from the web's cookie
session and admin auth. (An earlier React Native / Turborepo experiment was parked on the
`mobile-app-monorepo` branch — this repo is a plain single Next.js app, not a monorepo.)
